...
path2bb='/media/mra/win71/data/misc/kaggle/intel/data/train_annotations.hdf5'
...
# path train test
path2traintest='/media/mra/win71/data/misc/kaggle/intel/data/fold'+str(foldnm)+'_traintest_bb'+str(h)+'by'+str(w)+'.hdf5'
...
experiment='fold'+str(foldnm)+'_classify_bbcoordinates'+'_hw_'+str(h)+'by'+str(w)
...
def load_data(ids,data_type='train'):
    ...
    for k,id1 in enumerate(ids):
        ...
        try:
            ...
            # get bounding box
            x0,y0,w0,h0=ff_bb[id1]
            
            # crop ROI from image
            X0=X0[y0:y0+h0,x0:x0+w0]
