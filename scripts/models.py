...
def model(params):
    ...
    loss=params['loss']
    C=params['nb_filters']
    nb_output=params['num_classes']
    stride=params['stride']
    
    model = Sequential()
    model.add(Convolution2D(C, 3, 3, activation='relu',subsample=(stride,stride),border_mode='same', input_shape=(z, h, w)))
...
def model_fc(params):
    ...
    model = Sequential()    
    model.add(Dense(h1, activation='relu',input_dim=h0))
    model.add(Dropout(0.5))
...
# simple rnn for with features as input
def model_rnn(params):
    
    timesteps=params['timesteps']    
    input_length=params['input_length']
    nb_gru=params['nb_gru']
    lr=params['lr']
    
    # as the first layer in a Sequential model
    model = Sequential()
    model.add(GRU(nb_gru, input_shape=(timesteps, input_length)))
