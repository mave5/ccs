...
# path train test
#path2traintest='/media/mra/win71/data/misc/kaggle/intel/data/fold'+str(foldnm)+'_traintest'+str(h)+'by'+str(w)+'.hdf5'
path2traintest=path2root+'fold'+str(foldnm)+'_traintest'+str(h)+'by'+str(w)+'.hdf5'
...
if not os.path.exists(path2traintest):
    ...
    # get labels for stratify split of data
    y=[]
    for id in ids:
        #print id
        yk=ff_train[id]['y'].value
        y.append(yk)
            
    # train test split with stratify y
    ids_train, ids_test, _, _ = train_test_split(ids, y, test_size=0.1, random_state=123,stratify=y)

    X_train,y_train,_=load_data(ids_train)
    ff_traintest=h5py.File(path2traintest,'w')
    ff_traintest['X_train']=X_train
    ff_traintest['y_train']=y_train    
    
    X_test,y_test,_=load_data(ids_test)
...
#        hist = model.fit(X_batch, y_batch,verbose=0)
#        batches += 1
...
# load, crop, resize and save leaderboard data
