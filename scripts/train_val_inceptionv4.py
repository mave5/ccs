...
# path train test
path2traintest='/media/mra/win71/data/misc/kaggle/intel/data/fold'+str(foldnm)+'_traintest.hdf5'
...
experiment='fold'+str(foldnm)+'_inceptionv4'+'_hw_'+str(h)+'by'+str(w)
...
model = create_inception_v4(params_train)
...
for epoch in range(params_train['nbepoch']):
    ...
    for k in range(0,len(X_train),bs2):
        ...
        # normalization
        X_batch[:,0,:]=X_batch[:,0,:]-mean_X1
        X_batch[:,1,:]=X_batch[:,0,:]-mean_X2
        X_batch[:,2,:]=X_batch[:,0,:]-mean_X3
